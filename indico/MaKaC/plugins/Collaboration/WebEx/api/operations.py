...
<body>
  <bodyContent xsi:type="java:com.webex.service.binding.meeting.CreateMeeting" xmlns:meet="http://www.webex.com/schemas/2002/06/service/meeting" >
    ...
    <enableOptions>
      ...
      <chatHost>%(enableChat)s</chatHost>
      <chatPresenter>%(enableChat)s</chatPresenter>
...
""" % ( { "username" : params['webExUser'], "password" : escape(params['webExPass']), "siteID" : getWebExOptionValueByName("WESiteID"), "partnerID" : getWebExOptionValueByName("WEPartnerID"), "meetingPassword": escape(params['accessPassword']), "startDate" : start_date, "duration" : booking.getDuration(), "meetingName" : escape(params['meetingTitle']), "description" : escape(params['meetingDescription']), "participants": participant_xml, "openTime":escape(params['joinBeforeTime']), "enableChat": booking.getEnableChat(), "seeParticipants": booking.getSeeParticipants(), "joinBeforeHost": booking.getJoinBeforeHost() } )
...
""" % ( { "username" : params['webExUser'], "password" : escape(params['webExPass']), "siteID" : getWebExOptionValueByName("WESiteID"), "partnerID" : getWebExOptionValueByName("WEPartnerID"), "meetingPassword": escape(params['accessPassword']), "startDate" : start_date, "duration" : int(duration), "meetingName" : escape(params['meetingTitle']), "meetingKey" : booking._webExKey, "description": escape(params["meetingDescription"]), "participants": makeParticipantXML(booking._participants), "openTime":escape(params['joinBeforeTime']), "enableChat": booking.getEnableChat(), "seeParticipants": booking.getSeeParticipants(), "joinBeforeHost": booking.getJoinBeforeHost() } )
        response_xml = sendXMLRequest( request_xml )
        dom = xml.dom.minidom.parseString( response_xml )
        status = dom.getElementsByTagName( "serv:result" )[0].firstChild.toxml('utf-8')
        if status != "SUCCESS":
